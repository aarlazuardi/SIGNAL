...
export function getAuthHeaders(additionalHeaders = {}) {
  ...
  if (token) {
    ...
    console.log(
      `[API] Using token for API call - Length: ${
        token.length
...
export async function waitForAuthToken(maxWaitMs = 10000, intervalMs = 100) {
  ...
  let attempts = 0;
  let lastToken = null;
  let consecutiveValidTokens = 0; // Untuk memastikan token stabil

  while (Date.now() - startTime < maxWaitMs) {
    attempts++;
    let token = null;
    
    // Coba semua lokasi penyimpanan yang mungkin
    if (typeof window !== 'undefined') {
      // Coba localStorage (prioritas utama)
      token = localStorage.getItem("signal_auth_token");
      
      // Coba sessionStorage jika tidak ada di localStorage
      if (!token) {
        token = sessionStorage.getItem("signal_auth_token");
      }
      
      // Coba cookie jika masih tidak ada (fallback terakhir)
    ...
    if (token) {
      ...
      try {
        ...
        if (!isValidFormat) {
          console.warn(
            `[API] Token format tidak valid: ${token.substring(0, 10)}...`
          );
        } else {
          // Verifikasi token stabil (sama dengan sebelumnya)
          if (token === lastToken) {
            consecutiveValidTokens++;
            
            // Jika token sudah stabil selama 2 polling berturut-turut, return saja
            if (consecutiveValidTokens >= 2) {
              console.log(`[API] Token stabil selama ${consecutiveValidTokens} polling, mengembalikan token.`);
              return token;
            }
          } else {
            // Reset jika token berubah
            consecutiveValidTokens = 1;
            lastToken = token;
          }
        }
      } catch (e) {
...
export async function fetchWithAuth(
  ...
  if (waitForToken) {
    const token = await waitForAuthToken(15000, 300); // Tunggu lebih lama (15 detik) dengan polling setiap 300ms
    if (!token) {
      throw new Error(
    ...
    if (!currentAuthHeader || !currentAuthHeader.includes(token)) {
      ...
      console.log(`[API] Token ditambahkan ke header Authorization, panjang: ${token.length}`);
    }
  }
  ...
  while (retries <= maxRetries) {
    try {
      ...
      const response = await fetch(urlWithTimestamp, fetchOptions);      // Log response status for debugging
      ...
      if (response.status === 401 && retries < maxRetries) {
        ...
        try {
          ...
          console.log(`[API] 401 response body: ${respText.substring(0, 200)}`);
        } catch (e) {}

        // Dapatkan token baru dan coba lagi dengan delay lebih panjang
        removeAuthToken(); // Hapus token lama yang mungkin tidak valid
        await new Promise(resolve => setTimeout(resolve, 1000)); // Tunggu 1 detik sebelum coba mendapat token baru
        const newToken = await waitForAuthToken(10000, 200);
        
        if (newToken) {
          console.log(`[API] Mendapatkan token baru untuk retry: ${newToken.substring(0, 10)}...`);
          options.headers = options.headers || {};
        ...
        retries++;
        await new Promise(resolve => setTimeout(resolve, 500)); // Tunggu 0.5 detik sebelum retry
        continue;
      }
      ...
      if (retries < maxRetries) {
        retries++;
        // Tunggu lebih lama untuk setiap retry berikutnya (exponential backoff)
        const waitTime = 500 * Math.pow(2, retries); 
        console.log(`[API] Menunggu ${waitTime}ms sebelum mencoba lagi...`);
  ...
  console.error(`[API] Semua percobaan gagal untuk ${url}`);
  throw (
    lastError ||
