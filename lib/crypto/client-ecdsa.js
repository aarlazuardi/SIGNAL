 ...
 * @param {string} privateKeyBase64 - Kunci privat dalam format base64
 * @param {object} metadata - Metadata untuk ditandatangani bersama data (opsional)
 * @returns {Promise<string>} - Tanda tangan dalam format base64
 */
export async function signData(data, privateKeyBase64, metadata = {}) {
  try {
    // Import kunci privat
    const privateKey = await importPrivateKey(privateKeyBase64);

    // Jika ada metadata, gabungkan dengan data
    let dataToSign = data;
    if (metadata.perihal) {
      dataToSign = `${data}:${metadata.perihal}`;
    } else if (metadata.subject) {
      dataToSign = `${data}:${metadata.subject}`;
    }

    // Jika ada passHash, tambahkan juga
    if (metadata.passHash) {
      dataToSign = `${dataToSign}:${metadata.passHash}`;
    }
