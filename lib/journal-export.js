...
export function exportJournalToPDF(title, content, metadata = {}) {
  console.log("Memulai ekspor PDF...", { title, contentLength: content?.length });
  ...
  if (content && typeof content === 'string') {
    try {
      ...
      const isPDFHeader = content.trim().startsWith("JVBERi0") || content.trim().startsWith("JVBER");
      const isBase64Format = /^[A-Za-z0-9+/=\r\n]+$/.test(content.trim().slice(0, 100));
  ...
  if (isPDFBase64) {
    try {
      ...
      // Tambahkan timestamp ke nama file untuk mencegah cache
      const timestamp = new Date().getTime();
      link.download = `${title.replace(/[^a-zA-Z0-9]/g, "_")}_signed_${timestamp}.pdf`;
      ...
      return; // Keluar dari fungsi setelah mengunduh PDF asli
    } catch (error) {
      console.error("Error saat mengekstrak PDF asli:", error);
      console.log("Melanjutkan dengan metode pembuatan PDF baru...");
      // Lanjutkan dengan metode fallback jika gagal ekstrak PDF asli
    }
  }
  ...
  import("jspdf")
    .then(({ default: jsPDF }) => {
      try {
        ...
        if (metadata.originalFileType && ['pdf', 'doc', 'docx'].includes(metadata.originalFileType.toLowerCase())) {
          // Konten adalah dari file binary, tampilkan informasi
          doc.text("Dokumen ini adalah representasi dari file yang ditandatangani secara digital.", 20, yPosition);
          yPosition += 10;
          doc.text(`Tipe Dokumen Asli: ${metadata.originalFileType.toUpperCase()}`, 20, yPosition);
          yPosition += 10;
          doc.text("Detail tanda tangan digital dapat dilihat di halaman berikutnya.", 20, yPosition);
        } else {
          // Konten adalah teks biasa
          const textLines = doc.splitTextToSize(content || "Konten tidak tersedia", 170);
        ...
        if (metadata.signatureInfo) {
          ...
          doc.text("Dokumen ini telah ditandatangani secara digital dan terverifikasi.", 20, 280);
        }
